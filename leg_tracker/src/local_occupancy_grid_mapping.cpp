...
public:
  ...
  OccupancyGridMapping(ros::NodeHandle nh, string scan_topic):
    nh_(nh),
    ...
    sync(scan_sub_, non_leg_clusters_sub_, 100)
  {
    ...
    bool reverse_odom_reading;
    // nh_.param("scan_topic", scan_topic, std::string("scan")); // This param is new passed into constructor instead
    ...
    nh_private.param("reverse_odom_reading", reverse_odom_reading, false);
    ...
    //nh_private.param("decay", decay_, 0.05);
    decay_ = 0.0;
  ...
  // Note scan_intensity is not actual scan intensity but an indicator if the beam touches a human or not!
  bool isHuman(float scan_intensity)
  {
  ...
  void laserAndLegCallback(const sensor_msgs::LaserScan::ConstPtr& scan_msg, const leg_tracker::LegArray::ConstPtr& non_leg_clusters)
  {
    ...
    {
      // First step: find scan beams that correspond to tracked legs so we can count them as freespace in the grid occupancy map

      // Transform legs back into the laser frame    
      vector<tf::Point> non_legs;    
      for (int i = 0; i < non_leg_clusters->legs.size(); i++) // TODO this should be an iterator
      {
        ...
        {
          ROS_INFO("Local map tf err at position 1.1: %s", ex.what());
        }        
      }

      // Create a new scan which has intensities == 1 if the beam corresponds to a human and == 0 otherwise
      ...
      { 
        ...
        tf::Point c_pos = (*cluster)->getPosition();

        for (vector<tf::Point>::iterator non_leg = non_legs.begin(); 
        ...
        for (laser_processor::SampleSet::iterator s_iter = (*cluster)->begin();
          s_iter != (*cluster)->end();
          s_iter++)
        {
          scan.intensities[(*s_iter)->index] = is_leg; 
        }
       }

      // Get the latest transform from fixed_frame_->laser_frame
      bool transform_succesful = false;
      ...
      {        
        ROS_WARN("At position 2.1: %s", ex.what());
      ...
      if (transform_succesful)
      {
        ...
        // Get position of the local grid
        ...
        // Check if we need to shift the local grid
        if (sqrt(pow(grid_centre_pos_x_ - laser_x, 2) + pow(grid_centre_pos_y_ - laser_y, 2)) > shift_threshold_)
        {
          ...
          double* l_translated = new double [width_*width_];
          for (int i = 0; i < width_; i++)
          { 
            for (int j = 0; j < width_; j++)
            {
        ...
        { 
          for (int j = 0; j < width_; j++)
          {
            ...
            if (angle > scan.angle_min - scan.angle_increment/2.0 and angle < scan.angle_max + scan.angle_increment/2.0)
            {
              ...
              double closest_bream_angle = round(angle/scan.angle_increment)*scan.angle_increment;
              int closest_beam_idx = (int)round(angle/scan.angle_increment) + scan.ranges.size()/2;
              is_human = isHuman(scan.intensities[closest_beam_idx]);
              ...
              else if( !isfinite(scan.ranges[closest_beam_idx]) && scan.ranges[closest_beam_idx] < 0)
              {
              ...
              if (valid_measurement)
              {
                ...
                double angle_rel = angle - closest_bream_angle;
                if (dist > scan.range_max or dist > scan.ranges[closest_beam_idx] + ALPHA/2.0 or fabs(angle_rel)>BETA/2 or (!isfinite(scan.ranges[closest_beam_idx]) and dist > reliable_inf_range_))
                  m_update = UNKNOWN;
                else if (scan.ranges[closest_beam_idx] < scan.range_max and fabs(dist_rel)<ALPHA/2 and !is_human)   
                  m_update = OBSTACLE;
                else // if (dist < scan.ranges[closest_beam_idx]) //TODO should this just be an else?
                  m_update = FREE_SPACE;
              }
              else
              {
                // Assume cells corresponding to erroneous measurements are freespace. --> it's better to err on that side than to assume UNKNOWN
            ...
            double decay;
            if (is_human) 
              // Don't decay beams relating to people so any freespace behind them doesn't get decayed
              decay = 0.0;
            else
              decay = decay_;

            l_[i + width_*j] = (1-decay)*(l_[i + width_*j] + logit(m_update) - l0_);
        ...
        for (int i = 0; i < width_; i++)        // Note the opposite order because the OccupancyGrid message is row-major 
        {
          for (int j = 0; j < width_; j++)
          {
...
int main (int argc, char** argv)
{
  ...
  // We need to get 1 param, which is need for the initialization list of OccupancyGridMapping
  // TODO clearer way of doing this?
  ros::NodeHandle nh;
  string scan_topic;
